...
void BleUartComponent::setup() {
  ...
  if (!NimBLEDevice::init(this->device_name_))
  ...
  if (!NimBLEDevice::setMTU(517))
    ESP_LOGE(TAG, "BLE setMTU failed");
  ...
  tx_char_ = nus->createCharacteristic(NUS_TX_CHAR_UUID, NIMBLE_PROPERTY::NOTIFY);
